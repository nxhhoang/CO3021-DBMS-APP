...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{images/database_design/kienTrucUngDung.png}
...
\begin{itemize}[label=$\circ$]
    ...
    \item Thực thể \texttt{Address} (Địa chỉ người dùng - PostgreSQL). Mỗi địa chỉ gồm: \texttt{addressID} duy nhất, \texttt{addressLine}, \texttt{addressName}, \texttt{city}, \texttt{district}, \texttt{isDefault}. Mỗi người dùng có thể có nhiều địa chỉ giao hàng.
    ...
    \item Thực thể \texttt{Inventory} (Kho hàng - PostgreSQL). Mỗi bản ghi của kho gồm: \texttt{inventoryID} (duy nhất), \texttt{productID} (tham chiếu logic tới \texttt{Product} trong MongoDB), \texttt{sku} (tham chiếu logic tới \texttt{SKU} trong MongoDB), \texttt{stockQuantity}, \texttt{lastUpdated}. Một biến thể (\texttt{skuf}) có thể có nhiều bản ghi kho (ví dụ: được phân bổ ở các kho vật lý khác nhau).
    ...
    \texttt{paymentID} (duy nhất), \texttt{method} (\texttt{COD}, \texttt{BANKING}, \texttt{E\_WALLET}), \texttt{status} (\texttt{PENDING}, \texttt{COMPLETED}, \texttt{FAILED}, \texttt{REFUNDED}), \texttt{transactionDate}. 
    ...
    \item Thực thể \texttt{Category} (Danh mục - \texttt{MongoDB}): Mỗi danh mục bao gồm: \texttt{categoryID} (duy nhất), \texttt{name}, \texttt{slug} (duy nhất), \texttt{description}, \texttt{isActive}, và hai mảng bao gồm \texttt{dynamicAttributes} và \texttt{variantAttributes}. Trong đó, \texttt{dynamicAttributes} và \texttt{variantAttributes} lưu trữ cấu trúc các thuộc tính động cho phép của danh mục (bao gồm \texttt{key}, \texttt{label}, \texttt{dataType}, \texttt{options}). Mỗi danh mục có thể chứa nhiều sản phẩm.

    \item Thực thể \texttt{Product} (Hàng hoá - \texttt{MongoDB}). Mỗi sản phẩm có: \texttt{productID} duy nhất, \texttt{name}, \texttt{slug}, \texttt{categoryId} (tham chiếu logic tới thực thể Category), \texttt{basePrice}, \texttt{description}, \texttt{images}, \texttt{attributes} (dựa trên \texttt{dynamicAttributes} bao gồm các giá trị động ứng với \texttt{Category} mà \texttt{Product} thuộc về, \texttt{avg\_rating} (điểm đánh giá trung bình), \texttt{total\_reviews} (tổng số lượt đánh giá), \texttt{total\_sold} (tổng số lượng đã bán), \texttt{isActive}, \texttt{createdAt}. Một sản phẩm có thể nhận nhiều đánh giá (\texttt{review}). Sản phẩm không lưu trực tiếp số lượng tồn kho mà được tham chiếu \texttt{logic} sang \texttt{PostgreSQL}.

    \item Thực thể \texttt{sku} (Biến thể sản phẩm - MongoDB). Mỗi biến thể bao gồm: \texttt{sku} (duy nhất), \texttt{productID} (tham chiếu logic tới thực thể \texttt{Product}), \texttt{attributes} (các giá trị thuộc tính biến thể cụ thể, dựa trên \texttt{variantAttributes} của \texttt{Category}), \texttt{skuPrice} (giá của biến thể). Một sản phẩm có thể có nhiều biến thể (\texttt{SKU}) và một \texttt{SKU} có thể được tham chiếu bởi nhiều bản ghi kho (\texttt{Inventory}) trong PostgreSQL.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/database_design/EERD.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{images/database_design/mongoDB_EERD.png}
    \caption{Biểu đồ mô tả cho NoSQL database}
    \label{fig:mongoDB_EERD}
\end{figure}

\paragraph{Biểu đồ mô tả các quan hệ logic giữa SQL và NoSQL}\text{ }
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{images/database_design/logicRelationSQLAndNoSQL.png}
    \caption{Biểu đồ mô tả các quan hệ logic giữa NoSQL database và SQL database}
    \label{fig:logicRelationSQLAndNoSQL}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/database_design/mapping.png}
...
\begin{itemize}[label=$\circ$]
    \item Sản phẩm (\texttt{Products}) - Ảnh (\texttt{images}):
    \begin{itemize}[label=\textbullet]
        \item Quan hệ: 1 - Ít (Mỗi sản phẩm chỉ khoảng 5-10 ảnh).
        \item Quyết định: \texttt{Embed}. Lưu trực tiếp mảng \texttt{URL} ảnh trong \texttt{document} sản phẩm để lấy ra ngay lập tức.
    \end{itemize}
    \item Sản phẩm (\texttt{Products}) - Thuộc tính (\texttt{attributes}):
    \begin{itemize}[label=\textbullet]
        \item Quan hệ: 1 - 1 (Mỗi sản phẩm có 1 bộ thông số).
        \item Quyết định: \texttt{Embed}. Dữ liệu này luôn đi kèm sản phẩm và thay đổi cấu trúc tùy loại hàng (\texttt{Polymorphic Pattern}).
    \end{itemize}
    \item Sản phẩm - Đánh giá (reviews):
    \begin{itemize}[label=\textbullet]
        \item Quan hệ: 1 - Rất nhiều (Một sản phẩm có thể có hàng nghìn \texttt{review}).
        \item Quyết định: \texttt{Reference}. Nếu lồng \texttt{review} vào sản phẩm sẽ dễ bị tràn giới hạn \texttt{16MB} của \texttt{MongoDB}. Do đó, tách \texttt{Review} ra \texttt{collection} riêng và tham chiếu bằng \texttt{product\_id}.
    \end{itemize}
    \item Sản phẩm (\texttt{Products}) - Danh mục (\texttt{Category}):
    \begin{itemize}[label=\textbullet]
        \item Quan hệ: 1 - Nhiều (Một danh mục chứa rất nhiều sản phẩm, một sản phẩm thuộc một danh mục).
        \item Quyết định: \texttt{Reference} (Tham chiếu). Tách \texttt{Category} thành một \texttt{Collection} độc lập và tham chiếu qua \texttt{categoryId} trong \texttt{Product}. Lý do: \texttt{Category} đóng vai trò là danh mục gốc, được sử dụng chung cho nhiều sản phẩm và đóng vai trò lưu trữ khuôn mẫu cho các \texttt{dynamicAttributes}. Nếu \texttt{Embed} trực tiếp \texttt{Category} vào từng \texttt{Product} sẽ gây dư thừa dữ liệu khổng lồ và cực kỳ khó khăn khi cần cập nhật tên Danh mục hay cấu trúc thuộc tính động.
    \end{itemize}
    \item Sản phẩm (\texttt{Products}) - Biến thể (\texttt{SKUs}):
    \begin{itemize}[label=\textbullet]
        \item Quan hệ: 1 - Nhiều (Một sản phẩm có thể có nhiều biến thể khác nhau dựa trên các thuộc tính như dung lượng, màu sắc, kích thước,...).
        \item Quyết định: \texttt{Reference} (Tham chiếu). Tách \texttt{SKU} thành một \texttt{collection} độc lập và tham chiếu ngược lại \texttt{Product} thông qua \texttt{productID}. Lý do: Việc tách riêng giúp quản lý linh hoạt giá bán (\texttt{skuPrice}) và cấu hình thuộc tính riêng (\texttt{attributes}) của từng biến thể. Quan trọng hơn, thiết kế này giúp tối ưu việc tham chiếu logic với bảng \texttt{Inventory} dưới \texttt{PostgreSQL} để kiểm soát tồn kho cho từng mã SKU cụ thể mà không làm phình to \texttt{document} của sản phẩm gốc.
    \end{itemize}
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images/database_design/NoSQLCollections.drawio.png}
