...
\begin{itemize}[label=$\circ$]
    ...
    \item Thực thể \texttt{Category} (Danh mục - \texttt{MongoDB}): Mỗi danh mục bao gồm: \texttt{categoryID} (duy nhất), \texttt{name}, \texttt{slug} (duy nhất), \texttt{description}, \texttt{isActive}, và một mảng thuộc tính \texttt{dynamicAttributes}. Trong đó, \texttt{dynamicAttributes} lưu trữ cấu trúc các thuộc tính động cho phép của danh mục (bao gồm \texttt{key}, \texttt{label}, \texttt{dataType}, \texttt{isRequired}, \texttt{options}). Mỗi danh mục có thể chứa nhiều sản phẩm.

    \item Thực thể \texttt{Product} (Hàng hoá - \texttt{MongoDB}). Mỗi sản phẩm có: \texttt{productID} duy nhất, \texttt{name}, \texttt{slug}, \texttt{categoryId} (tham chiếu logic tới thực thể Category), \texttt{basePrice}, \texttt{description}, \texttt{images}, \texttt{attributes} (thuộc tính động như \texttt{RAM}, màu sắc, kích thước, cấu hình...), \texttt{avg\_rating} (điểm đánh giá trung bình), \texttt{total\_reviews} (tổng số lượt đánh giá), \texttt{total\_sold} (tổng số lượng đã bán), \texttt{isActive}, \texttt{createdAt}. Một sản phẩm có thể nhận nhiều đánh giá (\texttt{review}). Sản phẩm không lưu trực tiếp số lượng tồn kho mà được tham chiếu \texttt{logic} sang \texttt{PostgreSQL}.
\end{itemize}
