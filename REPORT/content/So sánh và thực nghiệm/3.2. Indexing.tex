...
\begin{itemize}[label=$\circ$]
    ...
    \begin{itemize}[label=$\bullet$]
        ...
        \item Mỗi sản phẩm có các thuộc tính chính: \texttt{id}, \texttt{categoryId}, \texttt{price}, \texttt{name}, \texttt{createdAt}.
    \end{itemize}
    
    \item \textbf{Truy vấn phổ biến:}
    \begin{itemize}[label=$\bullet$]
        \item Tìm kiếm sản phẩm theo \texttt{categoryId}.
...
\begin{itemize}[label=$\circ$]
    ...
    \begin{itemize}[label=$\bullet$]
        \item Tạo \textbf{B-tree index} trên các cột \texttt{categoryId} và \texttt{price} nhằm tăng tốc các truy vấn lọc và so sánh.
        \item Sử dụng \textbf{Multi-column B-tree index} trên (\texttt{categoryId}, \texttt{createdAt}) để tối ưu truy vấn lọc theo danh mục và sắp xếp theo thời gian.
...
\begin{itemize}[label=$\circ$]
    \item \textbf{Giải pháp:}
    \begin{itemize}
        \item Tạo \textbf{Single Field Index} trên \texttt{categoryId} để tăng tốc truy vấn lọc theo danh mục.
        \item Sử dụng \textbf{Compound Index} trên (\texttt{categoryId}, \texttt{createdAt}) nhằm tối ưu truy vấn lọc kết hợp sắp xếp.
