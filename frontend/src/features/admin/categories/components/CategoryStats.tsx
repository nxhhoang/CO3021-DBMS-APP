...
export const CategoryStats = ({
  ...
  return (
    <p className="max-w-2xl text-sm font-medium text-slate-500">
      ...
      <span className="font-bold text-slate-900">{totalCategories}</span> danh
      mục |{' '}
      <span className="font-bold text-slate-900">{totalDynamicAttributes}</span>{' '}
      thuộc tính động |{' '}
      <span className="font-bold text-slate-900">{totalVariantAttributes}</span>{' '}
      thuộc tính biến thể.
    </p>
  )
}
